...
public class StrictUnmodifiableTabulatedFunctionTest {
    ...
    @Test
    void getCount() {
        TabulatedFunction f1 = new StrictTabulatedFunction(new UnmodifiableTabulatedFunction(new ArrayTabulatedFunction(X1, Y1)));
        TabulatedFunction f2 = new UnmodifiableTabulatedFunction(new StrictTabulatedFunction(new LinkedListTabulatedFunction(X1, Y1)));
    ...
    void apply() {
        ...
        // case2: interpolation
        final double x1 = (X1[1] + X1[2]) / 2;
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x1));
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x1));

        // case3: extrapolate left
        final double x2 = X1[0] - 1;
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x2));
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x2));

        // case4: extrapolate right
        final double x3 = X1[X1.length - 1] + 1;
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x3));
        assertThrows(UnsupportedOperationException.class, () -> f1.apply(x3));
    }
}
